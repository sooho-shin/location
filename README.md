...
LOCATION은 사용자의 현재 위치와 관심 카테고리를 기준으로 한국의 장소를 추천하고 Google Maps 지도에 표시하는 웹 서비스입니다.

## 주요 기능

- 위치 기반 추천: 브라우저 Geolocation으로 현재 위치를 가져옵니다.
- AI 카테고리 추천: 현재 위치와 시간대에 맞는 탐색 주제를 바텀시트에 제안합니다.
...
- 실제 장소 사진: Google Places Photo가 있으면 상세 패널 상단에 실제 장소 이미지를 표시합니다.
- 지도 표시: Google Maps JavaScript API로 장소 마커와 클러스터를 표시합니다.
...
이 프로젝트의 제품 방향과 개발 기준은 `docs/` 아래 문서를 우선합니다.

- `docs/PRODUCT_SPEC.md`: 제품 기획, 사용자 흐름, 추천/지도/UI 정책
- `docs/DEVELOPMENT_GUIDE.md`: 개발 원칙, 환경 변수, 검증 기준
- `docs/API_CONTRACT.md`: 프론트엔드와 백엔드 API 계약
- `docs/ROADMAP.md`: 단계별 개발 방향

기능 동작이나 API 계약을 바꾸는 경우 관련 `docs/` 문서를 함께 갱신합니다.
