...
app.post("/api/categories/recommend", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      categories: getFallbackCategories(new Date().toISOString(), []),
      source: "fallback",
    });
  }
});
...
async function recommendCategoriesWithGemini(params: {
  ...
  const fallback = getFallbackCategories(params.localTime, params.recentSelectedCategoryIds);
  ...
  try {
    ...
    const text = result.response.text();
    const selected = parseCategoryResponse(text);
...
후보 카테고리:
${JSON.stringify(categoryPool, null, 2)}
...
- subtitle과 reason은 한국어로 짧게 다듬어도 됩니다.
- 현재 시간대와 여행자 맥락을 반영하세요.
- JSON 외의 설명, 마크다운, 코드블록을 넣지 마세요.
`;
}

function parseCategoryResponse(text: string): RecommendedCategory[] {
  const jsonText = extractJsonObject(text);
  ...
  const poolById = new Map(categoryPool.map((category) => [category.id, category]));
...
function getFallbackCategories(localTime: string, recentSelectedCategoryIds: string[]): RecommendedCategory[] {
  ...
  const preferred = preferredIds
    .map((id) => categoryPool.find((category) => category.id === id))
    .filter((category): category is RecommendedCategory => Boolean(category));

  const categories = [
    ...preferred.filter((category) => !recent.has(category.id)),
    ...preferred.filter((category) => recent.has(category.id)),
    ...categoryPool.filter((category) => !preferredIds.includes(category.id)),
  ];
